...
export default function UsedTrucks() {
  ...
  const getStockData = async () => {
    setLoading(false);
    const url = 'https://place.pixserver.click/api/autotrader/stock?page=' + page + '&pageSize=ALL';
    // const url="https://place.pixserver.click/api/autotrader/stock?page=1&pageSize=10";
    await axios.get(url, { mode: 'no-cors' })
      .then((response) => {
        if (response.status === 200) {
          setShowlist(response.data.data.results);
          setViewlist(response.data.data.results.slice(0,9));
          setTotallist(response.data.data.results);
          setLoading(true);
...
const more=()=>{
  ...
  if(a.length>=9)
    setViewlist([...viewlist,...a.slice(0,9)]);
  else{
    setVisible(false);
  ...
  useEffect(()=>{
    if(search.color==='*'){
      ...
      setViewlist(totallist.slice(0,9))  ;
      setVisible(totallist.length>9);
    }
    else {
      let b=totallist.filter((item)=>search.color.includes(item.vehicle.colour.toLowerCase()));
      setShowlist(b);    
      setViewlist(b.slice(0,9))  ;
      setVisible(b.length>9);
    }
     // eslint-disable-next-line react-hooks/exhaustive-deps
  },[search])
  ...
  return (
    <MainLayout>
      ...
      />
      <FilterModal open={open} setOpen={setOpen} data={reviewData}  setSearch={setSearch}/>
      <div className="flex pt-10 flex-col md:flex-row justify-center w-full bg-[#f6f6f6]">
        <div className="flex flex-col md:flex-row md:justify-between max-w-[1440px] ">
          <div className="px-5 md:block hidden min-w-[380px]  md:w-1/4">
            <RefineSearch setSearch={setSearch}/>
          </div>
...
const FilterModal = ({ open, setOpen, data , setSearch}) => {
